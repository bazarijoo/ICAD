...
class CausalMemoryDecoder(TransformerDecoderLayer):
    ...
    def __init__(self, d_model: int, num_heads: int, sequence_len: int, 
                 d_feedforward: int = 2048, kdim: int=128, vdim: int=128, dropout: float = 0.1,
                 layer_norm_eps: float = 1e-5, bias: bool = True) -> None:
        Module.__init__(self)

        self.multihead_attn = MultiheadAttention(d_model, num_heads, dropout=dropout, batch_first=True, bias=bias, kdim=kdim, vdim=vdim)
    ...
    def forward(self, tgt, memory):
        ...
        memory_mask = self.causal_memory_mask[:memory.shape[1], :memory.shape[1]].bool()
